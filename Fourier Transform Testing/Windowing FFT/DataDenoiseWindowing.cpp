...
vector<double> applyWindow(vector<double> data, vector<double> window) {

    size_t N = data.size();
    vector<double> fhatAppliedWindow(N);
...
int main() {
    ...
    int N = 1 << 12; //number of datapoints (2^10)
    int startTime = 0;
    int endTime = 1;
    double dt = (endTime - startTime) /( double)N;
    ...
    vector<double> dataWindowed = applyWindow(data, hannFunction(N)); //applying a window to the given data

    vector<complex<double>> fhat = fft(dataWindowed);    //computing the DFT
    ...
    size_t n = fhat.size(); //computes the size of the dataset
   ...
   //Creating file with data to be graphed in excel, note: DELETE 'Graphs.txt' TO REGENERATE
    ...
    fout << "Time" << '\t' << "Amplitude(Raw)" << '\t' << '\t' << "Time" << '\t' << "Amplitude(windowed)" << '\t' << '\t' << "Hz" << '\t' << "PSD" << '\t' << '\t' << "Time" << '\t' << "Amplitude(denoised)" << endl;
    cout << "Time" << '\t' << "Amplitude(Raw)" << '\t' << '\t' << "Time" << '\t' << "Amplitude(windowed)" << '\t' << '\t' << "Hz" << '\t' << "PSD" << '\t' << '\t' << "Time" << '\t' << "Amplitude(denoised)" << endl;

    for (size_t i = 0; i < n; i++) {
        double t = i*dt;
        // '\t' means tab which excel takes to mean a new collumn
        fout << t << '\t' << data[i] << '\t' << '\t' << t << '\t' << dataWindowed[i] << '\t' << '\t' << freq[i] << '\t' << psd[i] << '\t' << '\t' << t << '\t' << denoisedData[i].real() << endl; //send to file
        cout << t << '\t' << data[i] << '\t' << '\t' << t << '\t' << dataWindowed[i] << '\t' << '\t' << freq[i] << '\t' << psd[i] << '\t' << '\t' << t << '\t' << denoisedData[i].real() << endl; //send to console
    }
    fout.close();
